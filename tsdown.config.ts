/**
 * tsdown build for dsh-model-usage: the host-half lib (lib/index.js, ESM
 ...
 * - externals resolve through the loader module table at runtime (plugin is
 *   injected after @deepseek-ai/dsh-client-runtime / -ui-settings / -ui-slots,
 *   which come from the platform), so `react`, cordis and the DSH client
 *   kits are all external,
 ...
 * - the single CSS module compiles to a hashed class map and injects a
...
/** The shared client-bundle purity gate (see the doc comment on `makeConfig`). */
...
const PLUGIN_ID = 'dsh-model-usage'
